const
	...
	getObservedOf = function (item, key, parent) {
		...
		} else if (item instanceof Date || item instanceof Buffer || item instanceof Error) {
			return item;
		} else {
...
class Observable {
	...
	static from(target) {
		...
		} else if (target instanceof Date || target instanceof Buffer || target instanceof Error) {
